...
export function AccountDrawer(props: Props) {
  return (
    <Drawer
      ...
      title={
        props.mode === "add"
          ? vi.drawer.addAccountTitle
          : vi.drawer.editAccountTitle
      }
      classNames={{
    ...
    >
      <Stack gap="sm">
        <NativeSelect
          ...
          onChange={(e) =>
            props.onPlatformChange(e.currentTarget.value as Platform)
          }
          data={platformOptions}
          disabled={props.mode === "edit"}
        />
        <TextInput
          label={vi.drawer.displayName}
          value={props.displayName}
          onChange={(e) => props.onDisplayNameChange(e.currentTarget.value)}
        />
        <TextInput
          label={vi.drawer.serviceUrl}
          leftSection={<IconBrowser size={14} />}
          value={props.serviceUrl}
          onChange={(e) => props.onServiceUrlChange(e.currentTarget.value)}
        />
        <Switch
          label={vi.drawer.muteNotificationsPerAccount}
          checked={props.muted}
          onChange={(e) => props.onMutedChange(e.currentTarget.checked)}
        />
        <TextInput
          label={vi.drawer.userAgentOptional}
          value={props.userAgent}
          onChange={(e) => props.onUserAgentChange(e.currentTarget.value)}
        />
        <TextInput
          label={vi.drawer.proxyUrlOptional}
          value={props.proxyUrl}
          onChange={(e) => props.onProxyUrlChange(e.currentTarget.value)}
        />
        ...
        {props.mode === "edit" && (
          <Button
            ...
            onClick={props.onResetDefaultUrl}
          >
            {vi.drawer.resetDefaultUrl}
          </Button>
        )}

        <Button onClick={props.onSubmit} loading={props.busy}>
          {props.mode === "add" ? vi.drawer.addAccount : vi.drawer.saveChanges}
        </Button>
        ...
        <Box className="status-box">
          <Text size="xs" c="#9eb0d9">
            {vi.drawer.status}
          </Text>
          <Text size="sm" c="#dfe7ff">
            {props.statusText}
          </Text>
        </Box>
      </Stack>
